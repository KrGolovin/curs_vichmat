...
/*
   ...
   -------     2.0 March    1989      reltol, abstol added
               ...
               2.3 7  August  89      fixed true_v min. check
...
while (!found && !quit)
  ...
  for (jcount = 1; jcount <= konvge; ++jcount)
    ...
    (*icount)++;
    reflok = (ystar < ylo);    /* -- true_v if we have moved downhill -- */
    ...
    if (reflok) /* then */
      ...
      (*icount)++;
      extnok = (y2star < ystar);  /* -- true_v if we continue to
                                     move downhill -- */
	...
	for (i = 0; i < n; ++i)
        ...
        y2star = (*Fnelmin) (n, p2star);
        (*icount)++;
        contok = (y2star <= y[ihi]);  /* -- true_v if we have not
                                         gone uphill -- */
...
ok = true_v;   /* innocent until proven guilty */
for (i = 0; i < n; ++i)
...
found = false_v;   /* -- we did not find a true_v minimum -- */
for (i = 0; i < n; ++i) start[i] = xmin[i];
del = delta;
(*numres)++;
goto First;
